"""Evaluate QuantileCostAwarePolicy (cost-aware training) on 8-fold CV+VAL."""
...
from policies import QuantileCostAwarePolicy  # noqa: E402


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--target-quantile", type=float, default=0.833)
    ap.add_argument("--mult", type=float, default=1.0)
    ap.add_argument("--w-ma", type=float, default=0.329)
    ...
    name = args.name or f"qcap_q{args.target_quantile}_m{args.mult}_w{args.w_ma}"
    full_sales, full_in_stock, master = build_extended_sales()

    def factory():
        return QuantileCostAwarePolicy(
            target_quantile=args.target_quantile,
            multiplier=args.mult, w_ma=args.w_ma,
            censoring_strategy="mean_impute", random_state=42, master=master,
            point_n_variants=5,
        )

    print(f"[{name}] folds={list(folds_dict)} val={include_val} q={args.target_quantile} mult={args.mult} w_ma={args.w_ma}")
    t0 = time.time()
    results = score_policy_on_folds(
        ...
        coverage_weeks=3, alpha=args.target_quantile, n_workers=1,
    )
    wall = time.time() - t0
    ...
    log_experiment(name, results, extra={
        "target_quantile": args.target_quantile, "mult": args.mult, "w_ma": args.w_ma,
        "approach": "quantile_diverse_5model_pinball",
