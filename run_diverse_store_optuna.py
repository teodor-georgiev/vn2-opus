"""Optuna tuner for 5-model Diverse with per-store multipliers.
...
def main():
    ...
    ap.add_argument("--out", default="optuna_diverse5_store.json")
    args = ap.parse_args()
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        def factory(a=alpha, ml=mult_low, mh=mult_high, w=w_ma):
            return DiverseCostAwarePolicy(
                alpha=a, multiplier=1.0,
                mult_high=mh, mult_low=ml, high_demand_quantile=args.high_q,
                bin_basis="store",   # per-STORE aggregate demand split
                w_ma=w,
